...
# perform uorf analysis, result in a table displaying the number of reads for the gene with uorf as well as the reads number of 50-nt upstream of the stop codon of uorf
        ...
        for file in f:
            ...
            with open(file.strip("Aligned.sortedByCoord.out.bed") + 'uorf.csv', 'w') as out:
                ...
                for ID in id_list:
                    ...
                    c_list = []
                    # Traverses all uorfs, including overlapping uorfs
...
# get a heatmap showing the distribution of reads
def get_uorf_heatmap(file_name):
    ...
    data.pop("TSI")
    data.pop('gene name')
    scaler = preprocessing.StandardScaler().fit(data.T)  # Rows are normalized
